         ...
         | uu____2514 ->
             let uu____2515 =
               let uu____2517 =
                 let uu____2518 =
                   FStar_Util.format1 "<start constructor %s>" name  in
                 Caption uu____2518  in
               uu____2517 :: cdecl :: cid :: projs  in
             let uu____2519 =
               let uu____2521 =
                 let uu____2523 =
                   let uu____2524 =
                     FStar_Util.format1 "</end constructor %s>" name  in
                   Caption uu____2524  in
                 [uu____2523]  in
               FStar_List.append [disc] uu____2521  in
             FStar_List.append uu____2515 uu____2519)
                      ...
                      | (names,binders,n1) ->
                          ...
                          let prefix2 =
                            ...
                            | FStar_Pervasives_Native.Some p ->
                             ...
                             in
                          let nm =
                            let uu____2609 = FStar_Util.string_of_int n1  in
                            Prims.strcat prefix2 uu____2609  in
                          let names1 = (nm, s) :: names  in
                          let b =
                            let uu____2617 = strSort s  in
                            FStar_Util.format2 "(%s %s)" nm uu____2617  in
                          (names1, (b :: binders),
  ...
  fun enclosing_name  ->
    fun t  ->
      let next_qid =
        ...
        fun depth  ->
          ...
          then enclosing_name
            ...
            (let uu____2713 = FStar_Util.string_of_int n1  in
             FStar_Util.format2 "%s.%s" enclosing_name uu____2713)
         in
                ...
                FStar_All.pipe_right ps
                  (FStar_List.map
                     (fun tm  ->
                        ...
                        | App
                            (Var
                             "Prims.guard_free",{ tm = BoundV uu____2735;
                                                  freevars = uu____2736;
                                                  rng = uu____2737;_}::[])
                            -> tm
      ...
      let rec aux' depth n1 names t1 =
        ...
        | Integer i -> i
        | BoundV i ->
            let uu____2781 = FStar_List.nth names i  in
            FStar_All.pipe_right uu____2781 FStar_Pervasives_Native.fst
        | FreeV x -> FStar_Pervasives_Native.fst x
        | App (op,[]) -> op_to_string op
        | App (op,tms) ->
            let uu____2791 =
              let uu____2792 = FStar_List.map (aux1 n1 names) tms  in
              FStar_All.pipe_right uu____2792 (FStar_String.concat "\n")  in
            FStar_Util.format2 "(%s %s)" (op_to_string op) uu____2791
        | Labeled (t2,uu____2796,uu____2797) -> aux1 n1 names t2
        | LblPos (t2,s) ->
            let uu____2800 = aux1 n1 names t2  in
            FStar_Util.format2 "(! %s :lblpos %s)" uu____2800 s
        | Quant (qop,pats,wopt,sorts,body) ->
            let qid = next_qid ()  in
            let uu____2815 =
                              ...
                              (fun pats2  ->
                                 let uu____2854 =
                                   let uu____2855 =
                                     FStar_List.map
                                       (fun p  ->
                                          let uu____2858 = aux1 n2 names1 p
                                             in
                                          FStar_Util.format1 "%s" uu____2858)
                                       pats2
                                      in
                                   FStar_String.concat " " uu____2855  in
                                 FStar_Util.format1 "\n:pattern (%s)"
                                   uu____2854))
                          in
...
let rec declToSmt : Prims.string -> decl -> Prims.string =
  fun z3options  ->
    fun decl  ->
      ...
      | Caption c ->
          let uu____2985 = FStar_Options.log_queries ()  in
          if uu____2985
          then
            let uu____2986 =
              FStar_All.pipe_right (FStar_Util.splitlines c)
                (fun uu___91_2988  ->
                   match uu___91_2988 with | [] -> "" | h::t -> h)
               in
            FStar_Util.format1 "\n; %s" uu____2986
          else ""
      | DeclFun (f,argsorts,retsort,c) ->
          let l = FStar_List.map strSort argsorts  in
          let uu____3002 = caption_to_string c  in
          let uu____3003 = strSort retsort  in
          FStar_Util.format4 "%s(declare-fun %s (%s) %s)" uu____3002 f
            (FStar_String.concat " " l) uu____3003
      | DefineFun (f,arg_sorts,retsort,body,c) ->
          let uu____3011 = name_macro_binders arg_sorts  in
          (match uu____3011 with
           | (names,binders) ->
               let body1 =
                 let uu____3029 =
                   FStar_List.map (fun x  -> mkFreeV x norng) names  in
                 inst uu____3029 body  in
               let uu____3036 = caption_to_string c  in
               let uu____3037 = strSort retsort  in
               let uu____3038 = termToSmt (escape f) body1  in
               FStar_Util.format5 "%s(define-fun %s (%s) %s\n %s)" uu____3036
                 f (FStar_String.concat " " binders) uu____3037 uu____3038)
      | Assume a ->
          let fact_ids_to_string ids =
            FStar_All.pipe_right ids
              (FStar_List.map
                 (fun uu___92_3049  ->
                    match uu___92_3049 with
                    | Name n1 ->
             ...
             in
          let fids =
            let uu____3054 = FStar_Options.log_queries ()  in
            if uu____3054
            then
              let uu____3055 =
                let uu____3056 = fact_ids_to_string a.assumption_fact_ids  in
                FStar_String.concat "; " uu____3056  in
              FStar_Util.format1 ";;; Fact-ids: %s\n" uu____3055
            else ""  in
          let n1 = escape a.assumption_name  in
          let uu____3060 = caption_to_string a.assumption_caption  in
          let uu____3061 = termToSmt n1 a.assumption_term  in
          FStar_Util.format4 "%s%s(assert (! %s\n:named %s))" uu____3060 fids
            uu____3061 n1
      | Eval t ->
          let uu____3063 = termToSmt "eval" t  in
          FStar_Util.format1 "(eval %s)" uu____3063
      | Echo s -> FStar_Util.format1 "(echo \"%s\")" s
      | RetainAssumptions uu____3065 -> ""
      | CheckSat  -> "(check-sat)"
      | GetUnsatCore  ->
          "(echo \"<unsat-core>\")\n(get-unsat-core)\n(echo \"</unsat-core>\")"
      | Push  -> "(push)"
    ...
    let bcons =
      let uu____3269 =
        ...
        FStar_All.pipe_right uu____3271
         ...
         in
      FStar_All.pipe_right uu____3269 (FStar_String.concat "\n")  in
    let lex_ordering =
...
let mk_Term_uvar : Prims.int -> FStar_Range.range -> term =
  fun i  ->
    fun r  ->
      let uu____3296 =
        let uu____3300 = let uu____3302 = mkInteger' i norng  in [uu____3302]
           in
        ("Tm_uvar", uu____3300)  in
      mkApp uu____3296 r
...
let boxTerm : sort -> term -> term =
  fun sort  ->
    fun t  ->
      ...
      | Ref_sort  -> boxRef t
      | uu____3349 -> raise FStar_Util.Impos
...
let unboxTerm : sort -> term -> term =
  fun sort  ->
    fun t  ->
      ...
      | Ref_sort  -> unboxRef t
      | uu____3356 -> raise FStar_Util.Impos
  ...
  fun t  ->
    ...
    | App
        (Var
         "Prims.b2t",{
                       tm = App
                         (Var "Prims.op_Equality",uu____3364::t1::t2::[]);
    ...
    | App
        (Var
         "Prims.b2t",{
                       tm = App
                         (Var "Prims.op_disEquality",uu____3375::t1::t2::[]);
                       freevars = uu____3378; rng = uu____3379;_}::[])
        -> let uu____3386 = mkEq (t1, t2) norng  in mkNot uu____3386 t.rng
    | App
        (Var
    ...
    | App
        ...
        ->
        let uu____3427 =
          let uu____3430 = unboxInt t1  in
          let uu____3431 = unboxInt t2  in (uu____3430, uu____3431)  in
        mkGTE uu____3427 t.rng
    | App
        (Var
        ...
        ->
        let uu____3442 =
          let uu____3445 = unboxInt t1  in
          let uu____3446 = unboxInt t2  in (uu____3445, uu____3446)  in
        mkGT uu____3442 t.rng
    | App
        (Var
         "Prims.b2t",{ tm = App (Var "Prims.op_AmpAmp",t1::t2::[]);
                       freevars = uu____3449; rng = uu____3450;_}::[])
        ->
        let uu____3457 =
          let uu____3460 = unboxBool t1  in
          let uu____3461 = unboxBool t2  in (uu____3460, uu____3461)  in
        mkAnd uu____3457 t.rng
    | App
        (Var
         "Prims.b2t",{ tm = App (Var "Prims.op_BarBar",t1::t2::[]);
                       freevars = uu____3464; rng = uu____3465;_}::[])
        ->
        let uu____3472 =
          let uu____3475 = unboxBool t1  in
          let uu____3476 = unboxBool t2  in (uu____3475, uu____3476)  in
        mkOr uu____3472 t.rng
    | App
        (Var
         "Prims.b2t",{ tm = App (Var "Prims.op_Negation",t1::[]);
                       freevars = uu____3478; rng = uu____3479;_}::[])
        -> let uu____3486 = unboxBool t1  in mkNot uu____3486 t1.rng
    | App (Var "Prims.b2t",t1::[]) ->
        let uu___93_3489 = unboxBool t1  in
        {
          tm = (uu___93_3489.tm);
          freevars = (uu___93_3489.freevars);
          rng = (t.rng)
        }
    | uu____3492 -> mkApp ("Valid", [t]) t.rng
...
let mk_HasTypeFuel : term -> term -> term -> term =
  fun f  ->
    fun v1  ->
      fun t  ->
        let uu____3521 = FStar_Options.unthrottle_inductives ()  in
        if uu____3521
...
let mk_String_const : Prims.int -> FStar_Range.range -> term =
  fun i  ->
    fun r  ->
      let uu____3585 =
        let uu____3589 = let uu____3591 = mkInteger' i norng  in [uu____3591]
           in
        ("FString_const", uu____3589)  in
      mkApp uu____3585 r
...
let mk_Precedes : term -> term -> FStar_Range.range -> term =
  fun x1  ->
    fun x2  ->
      fun r  ->
        let uu____3602 = mkApp ("Precedes", [x1; x2]) r  in
        FStar_All.pipe_right uu____3602 mk_Valid
...
let rec n_fuel : Prims.int -> term =
  fun n1  ->
    ...
    then mkApp ("ZFuel", []) norng
      ...
      (let uu____3619 =
         let uu____3623 =
           let uu____3625 = n_fuel (n1 - (Prims.parse_int "1"))  in
           [uu____3625]  in
         ("SFuel", uu____3623)  in
       mkApp uu____3619 norng)
    ...
    fun p2  ->
      fun r  ->
        ...
        | (FStar_Pervasives_Native.Some p11,FStar_Pervasives_Native.Some p21)
            ->
            let uu____3648 = mkAnd (p11, p21) r  in
...
let mk_or_l : term Prims.list -> FStar_Range.range -> term =
  fun l  ->
    fun r  ->
      let uu____3693 = mkFalse r  in
      FStar_List.fold_right (fun p1  -> fun p2  -> mkOr (p1, p2) r) l
        uu____3693
  
let mk_haseq : term -> term =
  fun t  ->
    let uu____3699 = mkApp ("Prims.hasEq", [t]) t.rng  in mk_Valid uu____3699
...
let rec print_smt_term : term -> Prims.string =
  fun t  ->
    ...
    | BoundV n1 ->
        let uu____3713 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "(BoundV %s)" uu____3713
    | FreeV fv ->
        FStar_Util.format1 "(FreeV %s)" (FStar_Pervasives_Native.fst fv)
    | App (op,l) ->
        let uu____3721 = print_smt_term_list l  in
        FStar_Util.format2 "(%s %s)" (op_to_string op) uu____3721
    | Labeled (t1,r1,r2) ->
        let uu____3725 = print_smt_term t1  in
        FStar_Util.format2 "(Labeled '%s' %s)" r1 uu____3725
    | LblPos (t1,s) ->
        let uu____3728 = print_smt_term t1  in
        FStar_Util.format2 "(LblPos %s %s)" s uu____3728
    | Quant (qop,l,uu____3731,uu____3732,t1) ->
        let uu____3742 = print_smt_term_list_list l  in
        let uu____3743 = print_smt_term t1  in
        FStar_Util.format3 "(%s %s %s)" (qop_to_string qop) uu____3742
          uu____3743
    | Let (es,body) ->
        let uu____3748 = print_smt_term_list es  in
        let uu____3749 = print_smt_term body  in
        FStar_Util.format2 "(let %s %s)" uu____3748 uu____3749

and print_smt_term_list : term Prims.list -> Prims.string =
  fun l  ->
    let uu____3752 = FStar_List.map print_smt_term l  in
    FStar_All.pipe_right uu____3752 (FStar_String.concat " ")
    ...
    FStar_List.fold_left
      (fun s  ->
         fun l1  ->
           let uu____3762 =
             let uu____3763 =
               let uu____3764 = print_smt_term_list l1  in
               Prims.strcat uu____3764 " ] "  in
             Prims.strcat "; [ " uu____3763  in
           Prims.strcat s uu____3762) "" l
